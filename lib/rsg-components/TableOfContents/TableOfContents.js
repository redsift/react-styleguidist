	...
	_createClass(TableOfContents, [
		{
			...
			value: function renderLevel(sections) {
				...
				var items = sections.map(function(section) {
					var children = [].concat(
						_toConsumableArray(section.sections || []),
						_toConsumableArray(section.components || [])
					);
					var sectionDepth = section.sectionDepth || 0;
					var childHashPath =
						sectionDepth === 0 && useHashId
							? hashPath
							: [].concat(_toConsumableArray(hashPath), [section.name]);
					return Object.assign({}, section, {
						heading: !!section.name && children.length > 0,
						content:
							children.length > 0 &&
							_this2.renderLevel(children, useRouterLinks, childHashPath, sectionDepth === 0),
					});
				});
